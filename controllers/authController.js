const {getGoogleOauthToken, getGoogleUser} = require("../utils/googeServer");
...
const createSendToken = (statusCode, user, res) => {
    ...
    const cookieOption = {
        expires: new Date(Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000), httpOnly: true // chỉ cho phép truy cập cookie thông qua HTTP và không cho phép truy cập thông qua JavaScript. Điều này giảm nguy cơ bị tấn công XSS (Cross-Site Scripting)
    };
...
exports.forgotPass = catchAsync(async (req, res, next) => {
    ...
    try {
        await sendEmail({
            email: user.email, subject: 'Your password reset token (valid for 10 min)', message
        });
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!user || !(await user.checkPass(req.body.passwordCurrent, user.password))) {
...
exports.googleOauthHandler = async (req, res, next) => {
    try {
        ...
        const {id,email, verified_email,name,given_name,family_name} = await getGoogleUser({
            id_token, access_token,
        });

        const user = await User.findOne({
            authGoogleId: id, authType: 'google'
        });
        console.log(user);
        if (user){
            req.user=user
        ...
        try {
            const newUser = await User.create({
                authType: 'google', email: email, authGoogleId: id, lastName: family_name, firstName:given_name
            });
            createSendToken(201,newUser,res)
